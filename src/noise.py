...
from src.utils.robust_noise import gen_noisy_clients_vector, LabelNoiseWrapper, DataNoiseWrapper
if client_datasets is None:
    logger.info(f'[SIMULATE] Create client datasets!')
    client_datasets = []
    validation_sets = []

    # ✅ 生成每个 client 的噪声强度（gamma_c_i）
...
from src.utils.robust_noise import add_gradient_noise_state_dict, gen_adv_dataset_img
def upload(self):
    ...
    if self.args.with_aux and self.modality != 'img+txt':
        ...
        ...
        # 你原来这里直接 return new_sd
        to_send = new_sd
    else:
        to_send = sd

    # ✅ GN：对 noisy client 的上传权重加噪
    if getattr(self.args, "noise", 0) == 3 and getattr(self, "is_noisy", False) and getattr(self, "noise_gamma", 0.0) > 0:
...
if (
    ...
    adv_ds = gen_adv_dataset_img(
        args=self.args,
        model=self.model,          # 这里已经 .to(device) 且可能 DataParallel 了
